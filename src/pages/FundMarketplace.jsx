...
const QuickCollections = ({ onSelect }) => (
    <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide mb-8">
        {CONSTANTS.COLLECTIONS.map(c => {
            const Icon = c.icon;
            return (
                <button key={c.id} onClick={onSelect} className="flex items-center gap-3 px-6 py-4 glass-panel rounded-2xl min-w-[180px] hover:bg-white/80 dark:hover:bg-white/10 transition-all group border border-transparent hover:border-indigo-500">
                    <div className={`p-2 rounded-xl bg-gray-50 dark:bg-black/20 ${c.iconColor} group-hover:scale-110 transition`}><Icon size={20} /></div>
                    <div className="text-left"><p className="font-bold text-sm dark:text-white">{c.label}</p><p className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">Collection</p></div>
                </button>
            )
        })}
    </div>
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {funds.map(f => (
                        <div key={f.code} className="glass-panel p-6 rounded-3xl hover:border-indigo-500 transition-all flex flex-col justify-between group">
                            <div><span className="text-[10px] font-black uppercase bg-indigo-50 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-300 px-3 py-1 rounded-lg mb-4 inline-block">{f.category}</span><h3 className="text-xl font-bold dark:text-white mb-2 leading-tight line-clamp-2">{f.name}</h3><p className="text-[10px] text-gray-500 uppercase font-bold mb-6">{f.house}</p></div>
                            <div className="flex justify-between items-end border-t dark:border-white/5 pt-6"><div><p className="text-[10px] text-gray-400 mb-1 font-bold uppercase">NAV (Live)</p><p className="text-2xl font-mono font-black dark:text-white">₹{f.nav}</p></div><button onClick={() => onInvestClick(f)} className="bg-gray-900 dark:bg-white text-white dark:text-black px-6 py-3 rounded-2xl font-bold text-sm hover:scale-105 transition shadow-lg">Buy Scheme</button></div>
                        </div>
                    ))}
                </div>
            )}
